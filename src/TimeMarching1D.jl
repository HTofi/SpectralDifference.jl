module TimeMarching1D
    ...
    """
        𝓛(U::Array{T,3}, Uₗ::Array{T,1}, Uᵣ::Array{T,1}, L::Array{T,2}, D₁::Array{T,2}, D₂::Array{T,2}, V::Array{T,2}) where T <: Float64
    ...
    * The Vandermonde matrix associated to solution points `V`
    ...
    """
    function 𝓛(U::Array{T,3}, Uₗ::Array{T,1}, Uᵣ::Array{T,1}, L::Array{T,2}, D₁::Array{T,2}, D₂::Array{T,2}, V::Array{T,2}) where T <: Float64

        _, n, N = size(U)
        F = global_flux(U, Uₗ, Uᵣ, L)
        Lₕ = -flux_divergence(F, D₂)

        ε = global_artificial_viscosity(U, ε₀, s₀, κ, V)
    ...
    * The time step `Δt`
    ...
    """
    function RK45_SSP(U::Array{T,3}, Uₗ::Array{T,1}, Uᵣ::Array{T,1}, L::Array{T,2}, D₁::Array{T,2}, D₂::Array{T,2}, V::Array{T,2}, Δt::T) where T <: Float64

        v₁ = U + 0.39175222700392*Δt*𝓛(U,Uₗ,Uᵣ,L,D₁,D₂,V)[1]
        v₂ = 0.44437049406734*U + 0.55562950593266*v₁ + 0.36841059262959*Δt*𝓛(v₁,Uₗ,Uᵣ,L,D₁,D₂,V)[1]
        v₃ = 0.62010185138540*U + 0.37989814861460*v₂ + 0.25189177424738*Δt*𝓛(v₂,Uₗ,Uᵣ,L,D₁,D₂,V)[1]

        Lₕ₃ = 𝓛(v₃,Uₗ,Uᵣ,L,D₁,D₂,V)[1]
